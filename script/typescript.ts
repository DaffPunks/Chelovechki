...
const MAX_COINS: number = 100;

class GameEngine{
    coinBag: CoinBag;
    ingredientArm: Ingredient;
    ingredientLeg: Ingredient;
    ingredientCucumber: Ingredient;
    boxArm1: BoxStaff;
    boxArm2: BoxStaff;
    boxLeg1: BoxStaff;
    boxLeg2: BoxStaff;
    boxCucumber: BoxStaff;
    canBeCooked: boolean;
     ...
     */
    private getNumEnding(iNumber, aEndings) {
        var sEnding, i;
        iNumber = iNumber % 100;
    ...
    private checkInventory() {
        function textColor(id: string, amount: number) {
                $(id).text("X " + amount);
                if(amount == 0)
    ...
    private checkBox() {
        function ableToInsert(box: BoxStaff, ingredient: Ingredient) {
            if (!box.itInserted) {
    ...
    private checkButton() {
        function transparentButton(id: string, trueOrFalse: boolean) {
            if(trueOrFalse){
    ...
    public setObjects(coinBag: CoinBag, ingredientArm: Ingredient, ingredientLeg : Ingredient, ingredientCucumber: Ingredient,
               boxArm1: BoxStaff, boxArm2 : BoxStaff, boxLeg1: BoxStaff, boxLeg2: BoxStaff, boxCucumber: BoxStaff) {
...
class CoinBag{
    ...
    public plusCoins(amount: number) {
        for (var i = 0; i < amount; i++) {
            if (this.coins != MAX_COINS) {
                this.coins++;

                let difference = 101 - this.coins;
                if (this.coins == 1) {
    ...
    private makeCoinDiv(margin: number, difference: number) {
...
class Ingredient{
    ...
    constructor(cost: number, sell: number){
        this.amount = 0;
        this.cost = cost;
        this.sell = sell;
    }

    public buyIngredient (coinBag: CoinBag) {
        if (coinBag.coins >= this.cost){
            coinBag.removeCoins(this.cost);
            this.amount++;
        }
    }
    public sellIngredient (coinBag: CoinBag) {
        if(this.amount != 0) {
            this.amount--;
...
class BoxStaff{
    ...
    public insertIngredient (ingredient: Ingredient) {
